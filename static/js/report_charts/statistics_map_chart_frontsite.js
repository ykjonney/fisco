var myChart = null;
var drill_title = null;
var belong_to_liuan_title_list = ['裕安区', "金安区", "叶集区", "霍邱县", "舒城县", "金寨县", "霍山县", "开发区"];
function showMap(result_data, geoCoordMap, mapName, label_type_name, race_cid, category) {
    ...
    for (i = 0; i < city_list.length; i ++){
        title_list.push(city_list[i].name)
    }
    myChart.on('click', function (param) {
        //下钻其实就是点击事件，切换脚本而已
        if (param.name) {

            if (param.name === "上海" || param.name === "北京" || param.name === "天津" || param.name === "重庆" || $.inArray(param.name, title_list) === -1 ) {
                return
            }
            var name = param.name;
            if(name.indexOf('市') !== -1){
            ...
            $.getJSON('../../../../static/js/echarts/map/json/mapdata/' + map_code_dict[name] + '.json', function (map_data) {
                echarts.registerMap(name, map_data)
            });
            ajaxPost(url, data, function (ret) {
                if (ret.code === 1 && ret.data_dict !== "") {
                    city_title = ret.data_dict['title'];
                    is_city_level = 1;
                    if(category == '人数')
                        city_people_data = ret.data_dict
                    else
                        city_attend_data = ret.data_dict
                    drill_down_district(ret.data_dict, 'province');
                    ...
                    drill_down_district_map([data_dict], geoDistrictMap,name, label_type_name);
                }
            });
            $('.map_back').removeClass('dis_none');
            drill_title = param.name;
        }
    });
}
...
function showProvinceChart(result_data, province, label_type_name, race_cid, category, area_category) {
    ...
    echart_bar.on('click', function (param) {
        ...
        if (param.name) {
            ...
            url = "/frontsite/report/drill/down/district/";
            ajaxPost(url, {'name': param.name, "race_cid": race_cid, 'category': category, 'area_category': area_category}, function (ret) {
                if (ret.code === 1 && ret.data_dict !== ""){
                    city_title = ret.data_dict['title'];
                    is_city_level = 1;
                    if(category == '人数')
                        ...
                        city_attend_data = ret.data_dict;
                    drill_down_district(ret.data_dict, area_category);
                    let data_dict = ret.data_dict;
                    let title =  data_dict['title'];
                    if(title.indexOf('市') !== -1){
                            title = title.substring(0, title.length - 1)
...
function showCityChart(result_data, province, label_type_name, race_cid, category) {
    if (province === "" || province == null || province === undefined || province !== 'china') {
        var province_city_dict = do_get_province_city_data(result_data);
        result_data = province_city_dict[province];
    }
...
// 展示到下钻到区的图表
function drill_down_district(result_data, area_category) {
    var barMinWidth = $("#province_statistics").outerWidth() * 0.6;
    var city_data_list = result_data.city_list;
    var data_list = get_bar_data(city_data_list);
    var city_list = data_list[0];
    var city_series_data_list = data_list[1];
    var option_bar = {
        title: {
            text: '全国各省份答题人数一览（人次）',
            show: false
    ...
    echart_bar.clear();
    echart_bar.setOption(option_bar);
    $('#back').addClass("dis_none");
    if(area_category !== "city"){
    echart_bar.on("click", function (param) {
        if(param.name && belong_to_liuan_title_list.indexOf(param.name) !== -1) {
            drill_down_town(param)
        }
    })}
}
function drill_down_district_map(result_data, geoDistrictMap,mapName, label_type_name) {
    let district_data_list = do_series_district_data(result_data);
    let district_dealed_data = district_data_list[0];
    var district_list = district_data_list[1];
    var district_times_sum_dict = district_data_list[2];
    var district_times_sum = district_times_sum_dict[mapName];
    if (mapName === "" || mapName == null || mapName === undefined || mapName !== 'china') {
        district_list = district_dealed_data[mapName];
    }
    option = {
            backgroundColor: backgroundColor,
            title: {
            ...
            series: deal_series(district_list, district_times_sum, mapName, label_type_name)
        };
    // 指定图表的配置项和数据
    // 使用刚指定的配置项和数据显示图表。
    if (myChart != null && myChart !== "" && myChart !== undefined) {
...
function do_series_district_data(result_data){
    var res = {};
    var city_district_list = [];
    var city_district_data_list = [];
    var all_district_list = [];
    var times_sum = 0;
    var times_sum_dict = [];
    if (result_data) {
        for (var i = 0; i < result_data.length; i++) {
            var district_list = result_data[i].city_list;
            var city = result_data[i].title;
            // province = deal_special_province(province);
            result_data[i].title = city;

            times_sum += result_data[i].data;
            if (city) {
                city_district_list.push(city);
                city_district_data_list.push(result_data[i].data);
                times_sum_dict[city] = result_data[i].data;
            }
            if (district_list) {
                res[city] = [];
                for (var j = 0; j < district_list.length; j++) {
                    var geoCoord = geoCoordMap[district_list[j].title];
                    if (geoCoord) {
                        res[city].push({
                            name: district_list[j].title,
                            value: geoCoord.concat(district_list[j].data)
                        });
                        all_district_list.push({
                            name: district_list[j].title,
                            value: geoCoord.concat(district_list[j].data)
                        });
                    }
    ...
    return [res, all_district_list, times_sum_dict];
}

function drill_down(name, race_cid, label_type_name, category){
            var title = name;
            if(name.indexOf('市') !== -1){
                name = name.substring(0, name.length - 1)
            }
...
function drill_down_town(param) {
    $(".chat_box").addClass('dis_none');
    $(".analysis_tabs ").addClass('dis_none');
    $('.data_slot').addClass('dis_none');
    $('.table_info').html("");
    url = "/frontsite/report/town/rank/";
    data = {
        'category': 'town',
        'race_cid': new_race_cid,
        "district_title": param.name
    };
    ajaxPost(url, data, function (ret) {
        if (ret.code === 1) {
            $(".table_info").html(ret.html);
            $('#back').removeClass('dis_none')
        }
    });
}