function showTendencyAccuracyCharts(result_data, series_name, line_color) {
    var init_data_list = init_result_data(result_data, series_name, line_color);
    var xAxis_data_list = init_data_list[0];
    ...
    if (myChart === undefined || myChart === null || myChart === "") {
        myChart = echarts.init(document.getElementById('tendency_accuracy_charts'));
    }
    ...
    if (option) {
        ...
        if (legend_data_list.indexOf(series_name) < 0) {
            ...
            if (option.xAxis.data === undefined || option.xAxis.data.length <= xAxis_data_list.length) {
                option.xAxis.data = xAxis_data_list
            }
...
function init_result_data(result_data, series_name, line_color) {
    var xAxis_data_list = [];
    var series_list = [];
    var series_data = [];
